...
def payload_fraction_objective(
    dv: np.ndarray, G0: float, ISP: np.ndarray, EPSILON: np.ndarray
) -> float:
...
def objective_with_penalty(
    dv, G0, ISP, EPSILON, TOTAL_DELTA_V, return_tuple=False
    ...
    try:
        ...
        stage_ratio_max_violations = np.maximum(
            0, stage_ratios - 1
        )  # Violations above 1

        # Normalize physical constraints by number of stages
        physical_constraint = (
            np.sum(stage_ratio_min_violations) + np.sum(stage_ratio_max_violations)
        ) / len(stage_ratios)
        ...
        if return_tuple:
            ...
            return (
                objective
                + dv_penalty * dv_constraint
                + physical_penalty * physical_constraint
            )
...
def get_constraint_violations(
    ...
    try:
        ...
        physical_constraint = float(
            np.sum(np.maximum(0, -stage_ratios))
...
class RocketStageOptimizer:
    ...
    def solve(self, initial_guess, bounds):
        ...
        try:
            ...
            logger.info(
                f"Successfully completed parallel optimization with {len(results)} solutions"
            )
            return results
