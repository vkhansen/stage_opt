...
class DifferentialEvolutionSolver(BaseSolver):
    ...
    def __init__(
        ...
        config: Dict,
        mutation_min: float = 0.4,
        mutation_max: float = 0.9,
    ...
    def initialize_population(self):
        ...
        self.logger.debug(
            f"Using min_stage_fraction={min_stage_fraction:.4f} for {self.n_stages} stages"
        )
        ...
        for i in range(self.population_size):
            ...
            while np.any(props < min_stage_fraction) and retry_count < max_retries:
                ...
                if retry_count % 10 == 0:
                    self.logger.debug(
                        f"Retrying initialization {retry_count} times for individual {i}"
                    )

            # If we hit max retries, redistribute remaining delta-V
            if retry_count >= max_retries:
                self.logger.warning(
                    f"Hit max retries for individual {i}, redistributing stages"
                )
    ...
    def solve(self, initial_guess, bounds):
        ...
        try:
            ...
            for generation in range(self.max_iterations):
                ...
                F = (
                    self.mutation_min
                    + (self.mutation_max - self.mutation_min) * (1 - progress) ** 2
                )
                ...
                for i in range(self.population_size):
                    ...
                    if trial_fitness <= fitness[i]:
                        if (
                            i < 2
                        ):  # Log successful replacements for first few individuals
                        ...
                        if trial_fitness < best_fitness:
                            ...
                            self.logger.debug(
                                f"Old best: {best_solution} (fitness: {best_fitness:.6f})"
                            )
                            self.logger.debug(
                                f"New best: {trial} (fitness: {trial_fitness:.6f})"
                            )
                ...
                if not improved:
                    ...
                    if stall_count >= self.stall_limit:
                        self.logger.info(
                            f"DE converged after {generation + 1} generations"
                        )
                        break
                ...
                if (generation + 1) % 10 == 0:
                    self.logger.info(
                        f"DE generation {generation + 1}/{self.max_iterations}, "
