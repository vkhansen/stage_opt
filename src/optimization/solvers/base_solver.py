...
class BaseSolver(ABC):
    ...
    def update_best_solution(
        self, x: np.ndarray, score: float, is_feasible: bool, violation: float
    ) -> bool:
        ...
        try:
            ...
            for iteration in range(self.max_projection_iterations):
                ...
                if n_unfixed == 0:
                    ...
                    logger.warning(
                        "All stages fixed at bounds - cannot satisfy constraints"
                    )
                    break
                ...
                stage_constraints = self.config.get("constraints", {}).get(
                    "stage_fractions", {}
                )
                if stage_constraints:
                    ...
                    min_first = (
                        first_stage.get("min_fraction", 0.15) * self.TOTAL_DELTA_V
                    )
                    max_first = (
                        first_stage.get("max_fraction", 0.80) * self.TOTAL_DELTA_V
                    )
                    x_proj[0] = np.clip(x_proj[0], min_first, max_first)

                    # Other stages constraints
                    min_other = (
                        other_stages.get("min_fraction", 0.01) * self.TOTAL_DELTA_V
                    )
                    max_other = (
                        other_stages.get("max_fraction", 1.0) * self.TOTAL_DELTA_V
                    )
    ...
    def initialize_population_lhs(self) -> np.ndarray:
        ...
        try:
            ...
            population = np.zeros(
                (self.population_size, self.n_stages), dtype=np.float64
            )
    ...
    def process_results(
        ...
        try:
            ...
            stage_ratios, mass_ratios = calculate_stage_ratios(
                dv=x, G0=self.G0, ISP=self.ISP, EPSILON=self.EPSILON
            )
            ...
            if is_feasible:
                for i, (dv, mr, sr) in enumerate(zip(x, mass_ratios, stage_ratios)):
                    stages.append(
                        {"stage": i + 1, "delta_v": float(dv), "Lambda": float(sr)}
                    )
