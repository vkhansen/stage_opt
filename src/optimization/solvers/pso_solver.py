...
class ParticleSwarmOptimizer(BaseSolver):
    ...
    def initialize_swarm(self):
        ...
        for i in range(self.population_size):
            ...
            while np.any(props < min_stage_fraction) and retry_count < max_retries:
                ...
                if retry_count % 10 == 0:
                    self.logger.debug(
                        f"Retrying initialization {retry_count} times for particle {i}"
                    )

            # If we hit max retries, redistribute remaining delta-V
            if retry_count >= max_retries:
                self.logger.warning(
                    f"Hit max retries for particle {i}, redistributing stages"
                )
    ...
    def solve(self, initial_guess, bounds):
        ...
        try:
            ...
            p_best_scores = np.full(
                self.population_size, float("inf"), dtype=np.float64
            )
            ...
            for iteration in range(self.max_iterations):
                ...
                for i in range(self.population_size):
                    ...
                    if max_stage > max_stage_dv:
                        self.logger.debug(
                            f"Rebalancing particle {i} - Stage exceeds limit:"
                        )
                        ...
                        if (
                            other_sum > 1e-10
                        ):  # Use small threshold to avoid division by zero
                            ...
                            self.logger.debug(
                                "Using equal redistribution due to near-zero sum"
                            )
                    ...
                    if score < p_best_scores[i]:
                        ...
                        if score < g_best_score:
                            ...
                            self.logger.debug(
                                f"Old best: {g_best_pos} (score: {g_best_score:.6f})"
                            )
                            self.logger.debug(
                                f"New best: {positions[i]} (score: {score:.6f})"
                            )
                            g_best_score = score
                ...
                if not improved:
                    ...
                    if stall_count >= self.stall_limit:
                        self.logger.info(
                            f"PSO converged after {iteration + 1} iterations"
                        )
                        break
