...
class GeneticAlgorithmSolver(BaseGASolver):
    ...
    def __init__(
        ...
        if config is not None:
            ...
            stagnation_generations = solver_params.get(
                "stagnation_generations", stagnation_generations
            )
            stagnation_threshold = solver_params.get(
    ...
    def _log_generation_stats(self, algorithm) -> Tuple[float, float, float]:
        ...
        try:
            ...
            if len(self.best_fitness_history) > 1:
                improvement = (
                    (self.best_fitness_history[-2] - best_fitness)
                    / abs(self.best_fitness_history[-2])
                    * 100
                )
            else:
                improvement = 0.0
            ...
            if diversity < self.min_diversity:
                logger.warning(
                    "  Low population diversity detected - possible premature convergence"
                )
            ...
            if len(self.best_fitness_history) > self.stagnation_generations:
                ...
                if recent_improvement < self.stagnation_threshold:
                    logger.warning(
                        "  Optimization appears to be stagnating - consider adjusting parameters"
                    )
    ...
    def solve(
        self, initial_guess: np.ndarray, bounds: List[Tuple[float, float]]
    ) -> dict:
        ...
        try:
            ...
            for gen in range(self.n_gen):
                ...
                logger.info(
                    f"Generation {gen + 1}/{self.n_gen} - Best: {gen_best_fitness:.6f}, Avg: {avg_fitness:.6f}, Diversity: {diversity:.6f}"
                )
